...
from .trainer import  make_evaluate, train
import datasets
...
def simple_train(
    ...
    per_device_batch_size = 4 if lora_params is not None else 2  # TODO(arjun): parameterize
    aggregate_device_batch_size = per_device_batch_size * accelerator.num_processes
    ...
    max_steps = max_steps // accelerator.num_processes

    accelerator.print(f"max_steps: {max_steps}")

    accelerator.init_trackers(project_name="train", config = {
    ...
    if lora_params is not None:
        model = starcoder_lora(
            model, **lora_params #lora_r=32, lora_alpha=32, lora_dropout=0.10
        )
        print_trainable_parameters(model)
    else:
        model = AutoModelForCausalLM.from_pretrained(model, torch_dtype=torch.float16, use_cache=False, device_map= {"": accelerator.device})
    ...
    evaluate = GenerateForEvaluate.from_jsonl(model, tokenizer, accelerator, Path(test_data),  batch_size=1, num_return_sequences=20)

    train_dataloader, test_dataloader, optim, lr_scheduler, model = accelerator.prepare(
        train_dataloader, evaluate.test_dataloader, optim, lr_scheduler, model
    )
    evaluate.test_dataloader = test_dataloader
